...
(function () {
  ...
  host.innerHTML = `
    ...
    <section class="detail-heading"><div class="detail-heading-copy"><span class="badge ${item.status}">${esc(STATUS[item.status].label)}</span><h1>${esc(name)}</h1><p class="detail-tagline">${esc(item.tagline || (planned?'面向行业工作的智能助手方案（规划）':'按岗位组合助手，解决日常业务问题'))}</p><p class="detail-description">${esc(item.desc)}</p></div>
      <aside class="detail-summary"><span>${planned?'方案状态':'云端订阅 · 建议价'}</span><strong>${planned?'规划中':money(price.monthly)}${planned?'':`<small>${isSet?'/ 5人团队 / 月':'/ 人 / 月'}</small>`}</strong><a class="btn btn-red" href="#deployment">${planned?'查看规划范围':'查看部署与价格'}</a>${trialReady?`<a class="detail-trial" href="${esc(trial)}" target="_blank" rel="noopener noreferrer">进入试用系统 ↗</a>`:planned?'':'<span class="trial-pending">线上试用入口待开放</span>'}</aside>
    </section>

(function () {
  ...
  host.innerHTML=`
    ...
    <section class="detail-heading employee-heading"><div><p class="section-kicker">DIGITAL WORKFORCE / ${line.id.toUpperCase()}</p><h1>数字员工 · ${esc(line.name)}</h1><p class="detail-tagline">${esc(line.tagline)}</p><p class="detail-description">${esc(line.desc)}</p><p class="employee-count">${esc(line.count)} <span>产品方案 · 按需交付</span></p></div><aside class="detail-summary"><span>${SITE.showPricing?'建议起步方案':'产品状态'}</span><strong>${SITE.showPricing?`${money(line.price)}<small>${esc(line.unit)}</small>`:'已上线'}</strong><a class="btn btn-red" href="#deployment">${SITE.showPricing?'查看费用与包含内容':'查看部署方式'}</a><span>先选岗位，再确定流程与对接范围</span></aside></section>
